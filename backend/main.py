...
app.add_middleware(
    CORSMiddleware,
    allow_origins=origins, # Update this with your Render URL later
...
def log_ui(msg):
    ...
    UI_LOGS.append(f"[{timestamp}] {msg}")
...
def get_gallery(user_id: str):
    # Fetch directly from DB. The 'filename' will now be a full URL.
    response = supabase.table("clips").select("*").eq("user_id", user_id).order("created_at", desc=True).execute()
    return response.data

@app.get("/video/{filename}")
def get_video(filename: str):
    # Backward compatibility for local files, but ideally frontend uses URL now
...
def process_video_pipeline(raw_file_path, user_id):
    ...
    try:
        ...
        for vid_path, title in final_videos:
            ...
            # --- CLOUD UPLOAD ---
            log_ui(f"☁️ Uploading {title} to Cloud...")
            ...
            if public_url:
                ...
                # SAVE URL TO DB (NOT LOCAL PATH)
                data = {
                    "user_id": user_id,
                    "filename": public_url, # <--- Saving URL now
                    "title": title,
                    "description": desc,
                    "score": score
                }
